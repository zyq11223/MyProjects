...
public class Draw_RTT3 extends ApplicationFrame {

	 private static final long serialVersionUID = 1L;

	 // 构造方法
	 public Draw_RTT3(String str) throws Exception{
	  super(str);
	  //生成图片步骤：
	 ...
	 public static CategoryDataset createDataset() throws Exception{
	  ...
	  // 放入数据
	  BufferedReader bReader1 = new BufferedReader(new FileReader("Ping_statistics.json"));
		BufferedReader bReader2 = new BufferedReader(new FileReader("Website.txt"));
		String[] strings = new String[4];
		double[] drop = new double[4];
		double[] maxRTT = new double[4];
		double[] medRTT = new double[4];
		...
		for(int i=0;i<4;i++) {
			JSONObject jo2 = JSONObject.fromObject(string2);
			JSONObject jo1 = jo2.getJSONObject(strings[i]);
			System.out.println(strings[i]);
			System.out.println(jo1);
			drop[i] = Double.parseDouble(jo1.get("drop_rate").toString().split("%")[0]);
			maxRTT[i] = Double.parseDouble(jo1.get("max_rtt").toString());
			medRTT[i] = Double.parseDouble(jo1.get("median_rtt").toString());
			
			
		}
	    			
					for (int k=0;k<4;k++) {
						dataset.addValue(drop[k],String.valueOf(k+1) ,strings[k]);
					}
	 ...
	 public static JFreeChart createChart(CategoryDataset dataset) {
	  // create the chart..
	  JFreeChart chart = ChartFactory.createBarChart("网站的丢包率柱状图",// 标题
	    "Website",// x轴
	    "百分比",// y轴
	 ...
	 public static void main(String[] argls) throws Exception{
		 Draw_RTT3 demo = new Draw_RTT3("MyDemo01 ");
	  demo.pack();
